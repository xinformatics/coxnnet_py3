...
########### Original Author: T. Ching https://traversc.github.io
## I have made tweaks into the original code so that it can run using Python3 and takes multiomic input
...
# alternatively use dill instead of pickle
import dill
...
class HiddenLayer(object):
    def __init__(self, rng, input, n_samples, map, label, activation=T.tanh):
        ...
        for i in range(len(map)):
            W_values = numpy.asarray(rng.uniform(low=-numpy.sqrt(6. / (map[i][0] + map[i][2])), 
            high=numpy.sqrt(6. / (map[i][0] + map[i][2])), size=(int(map[i][0]), int(map[i][2]))),dtype=theano.config.floatX)
...
class CoxMlp(object):
    def __init__(self, x_train, rng, n_samples, node_map, input_split):
        ...
        if node_map == None:
            #self.node_map = [[(x_train.shape[1],[0],numpy.ceil(2*(x_train.shape[1] ** 0.5)))]] #hidden layer nodes are 2*sqrt(input)
            self.node_map = [[(x_train.shape[1],[0],numpy.ceil(x_train.shape[1] ** 0.5))]] #original implimentation where hidden layer nodes are sqrt(input)
        else:
...
def crossValidate(x_train, ytime_train, ystatus_train, model_params = dict(),search_params = dict(),cv_params = dict(), verbose=False):
    ...
    #old cv_folds=cross_validation.KFold(N_train,n_folds=n_folds, shuffle=True, random_state=cv_seed)
